...
type IGateModule interface {
	...
	SendMsg(sId string, msg *[]byte) (result int, err string)                //发送消息到用户
	SendMsgByGroup(aIds []string, msg *[]byte) (result []string, err string) //发送消息到用户组
	SendMsgByBroadcast(msg *[]byte) (result int, err string)                 //发送消息到全服
}
...
type IAgentMgrComp interface {
	...
	Connect(a IAgent)
	DisConnect(a IAgent)
	SendMsg(aId string, msg *[]byte) (result int, err string)                //发送消息到用户
	SendMsgByGroup(aIds []string, msg *[]byte) (result []string, err string) //发送消息到用户组
	SendMsgByBroadcast(msg *[]byte) (result int, err string)                 //发送消息到全服
...
type IAgent interface {
	...
	WriteMsg(msg *[]byte) (err error)
	OnRecover(msg proto.IMessage)
	OnRecoverByte(ComId uint16, MsgId uint16, MsgLen uint32, Buffer *[]byte)
	RevNum() int64
	SendNum() int64
	IsClosed() bool
	OnRun()
