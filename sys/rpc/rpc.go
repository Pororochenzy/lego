...
import (
	...
	"errors"
	"fmt"
	"github.com/Pororochenzy/lego/sys/rpc/serialize"
	"reflect"
	"runtime"
	"strings"
	"sync"
	...
	"github.com/Pororochenzy/lego/sys/discovery"
	"github.com/Pororochenzy/lego/sys/event"
	"github.com/Pororochenzy/lego/sys/log"
	"github.com/Pororochenzy/lego/sys/rpc/connpool"
	"github.com/Pororochenzy/lego/sys/rpc/protocol"
	"github.com/Pororochenzy/lego/sys/rpc/rpccore"
	"github.com/Pororochenzy/lego/sys/rpc/selector"
)
...
// 旧版本兼容
func (this *rpc) Call_(_func string, params ...interface{}) (interface{}, error) {
	...
	for k, param := range params {
		...
		if err != nil {
			log.Errorf("RPC CallNR ServerId = %s f = %s  ERROR = %v ", this.options.ServiceNode.Id, _func, err)
	...
	if errstr != "" {
		log.Errorf("RPC Call ServerId = %s f = %s Elapsed = %v Result = %v ERROR = %v", this.options.ServiceNode.Id, _func, time.Since(start), r, errstr)
		return r, fmt.Errorf(errstr)
	} else {
		return r, nil
	}
}
func (this *rpc) CallNR(_func string, params ...interface{}) (err error) {
	return nil
}

func (this *rpc) CallArgs(_func string, ArgsType []string, args [][]byte) (interface{}, string) {
