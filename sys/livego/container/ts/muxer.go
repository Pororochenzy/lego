...
func (muxer *Muxer) Mux(p *core.Packet, w io.Writer) error {
	first := true
	wBytes := 0
	pesIndex := 0
	tmpLen := byte(0)
	...
	pts := dts
	pid := audioPID
	var videoH core.VideoPacketHeader
	if p.IsVideo {
		pid = videoPID
		videoH, _ = p.Header.(core.VideoPacketHeader)
...
func (header *pesHeader) packet(p *core.Packet, pts, dts int64) error {
	//PES header
	i := 0
	header.data[i] = 0x00
	i++
